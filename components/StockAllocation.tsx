...
export default function StockAllocation({
  ...
  // 1. Calculate Active Missing Products Aggregation (Separating Normal Pending & En Rupture)
  ...
  const handleMarkEnRupture = async (productName: string) => {
    ...
    try {
      ...
      setToastMessage(`تم وسم "${productName}" كمقطوع ونقلها لقسم السلع المقطوعة`);
  ...
  return (
    <div className="space-y-3 relative">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-2xs space-y-3">
        ...
        {/* Section Switcher: Normal Pending vs En Rupture */}
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-2xs space-y-2.5">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5 pb-2.5 border-b border-slate-100">
          <span className="text-base font-bold text-slate-800">
            {activeTab === 'normal' ? 'قائمة الخصاصات المطلوب توفيرها' : 'قائمة السلع المقطوعة (En rupture)'}
          </span>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            ...
            <div className="relative flex items-center">
              ...
              >
                <option value="alphabetical">أبجديا (أ - ي ثم A - Z)</option>
        ...
        {displayedProducts.length === 0 ? (
          <div className="text-center py-8 border border-dashed border-slate-200 rounded-lg bg-slate-50/50 space-y-1.5">
            {activeTab === 'normal' ? (
              ...
              <>
                <PackageCheck className="w-5 h-5 text-slate-400 mx-auto" />
                <p className="font-bold text-slate-700 text-xs">لا توجد سلع موسومة كمقطوعة حالياً</p>
              </>
            )}
          </div>
        ) : (
            ...
            {displayedProducts.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-2 w-full md:w-auto md:flex md:items-center mt-2.5 md:mt-0 pt-2 md:pt-0 border-t md:border-t-0 border-slate-100 justify-end">
                    
                    {/* Blue Ready (جاهز) Button */}
                    <button
                      onClick={() => handleOpenModal(item.productName, item.totalMissingQty)}
                      disabled={isProcessing}
                      className="h-8 px-3 text-xs font-semibold rounded-lg bg-blue-600 hover:bg-blue-700 text-white flex items-center justify-center gap-1.5 shadow-2xs transition-colors disabled:opacity-50 w-full md:w-auto shrink-0"
                    >
                      <CheckCircle2 className="w-3.5 h-3.5 text-white shrink-0" />
                      <span className="truncate">{isProcessing ? 'جاري...' : 'جاهز'}</span>
                    </button>

                    {/* In Normal Tab: Green 'مقطوع' Button */}
                    {activeTab === 'normal' ? (
                      <button
                        onClick={() => handleMarkEnRupture(item.productName)}
                        disabled={isProcessing}
                        title="وسم السلعة كمقطوعة ونقلها لقسم السلع المقطوعة"
                        className="h-8 px-3 text-xs font-semibold rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white flex items-center justify-center gap-1.5 shadow-2xs transition-colors disabled:opacity-50 w-full md:w-auto shrink-0"
                      >
                        <Ban className="w-3.5 h-3.5 text-white shrink-0" />
                        <span className="truncate">مقطوع</span>
                      </button>
                    ) : (
                      ...
                      <button
                        ...
                        className="h-8 px-3 text-xs font-semibold rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 flex items-center justify-center gap-1.5 shadow-2xs transition-colors disabled:opacity-50 w-full md:w-auto shrink-0"
                      >
